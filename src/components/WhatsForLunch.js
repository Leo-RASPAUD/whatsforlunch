...
export default () => {
  ...
  const getRestaurants = async () => {
    ...
    const result = await fetch(
      `${baseUrl}?location=${coordinates.latitude},${
        coordinates.longitude
      }&type=restaurant&keyword=${filter}&radius=${radius}`
    );
    const json = await result.json();
    ...
    <>
      ...
      {!loading && restaurant && (
        <div>
          ...
          <Text
            label="Phone number"
            value={restaurant.formatted_phone_number}
          />
          <Text label="Rating" value={restaurant.rating} />
          <Text
            label="Number of ratings"
            value={restaurant.user_ratings_total}
          />
          <OpeningHours openingHours={restaurant.opening_hours} />
          {restaurant.price_level && (
            <Text label="Price level" value={restaurant.price_level} />
          )}
